...
function filterMandatoryProps(body, mandatoryProps) {
  ...
  return mandatoryProps.filter((val) => !hasOwnProperty.call(body, val));
}
...
function expressty(req, res, next) {
  function respond(httpCode, payload, message, responseCode, jsonp) {
    if (jsonp) {
      res.status(httpCode).jsonp(generateResponseObject(httpCode, payload, message, responseCode));
      return;
    }
